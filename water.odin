...
water_volumes_from_end_points_pairs :: proc(
	cell: Cell,
	endpoints: [dynamic]Water_Endpoint,
	...
	for pair in pairs {
		...
		volume.min.x = math.min(pair.endpoints[0].position.x, pair.endpoints[1].position.x) + 1
			...
			case .West:
				...
				west_fat_loop: for ft in iter_fat_tiles(&iter) {
					...
					case bot.value == .Empty && top.value == .Empty:
						...
						append(
							&paths_to_execute,
							Water_Path{origin = top.relative_position, direction = .South},
						)
						continue paths_loop
